# GENERAL
hyp.drift = function(x, theta) {
  mu = theta[1]
  lambda = theta[2]
  delta = theta[3]
  sigma = theta[4]
   (- ((lambda) / (sqrt(delta^2 + (x-mu)^2)))) * (x - mu)
}

trans.hyp.fast = function(start, steps, theta, time = 1) {
  Y <- numeric(steps)
  Y[1] = start
  Z = rnorm(steps - 1)
  Dt <- time / (steps - 1)
  for (i in 1:(steps - 1)) {
    Y[i+1] = Y[i] + hyp.drift(Y[i] * theta[4], theta) / theta[4] * Dt + sqrt(Dt) * Z[i]
  }
  as.numeric(Y)
}
...
q.beskos.unknown = function(theta, r.diffusion, theta.tilde, r.nu, M, steps, horizons, tot.time, exact = F) {
  if (theta[2]<0 |theta[3]<0 | theta[4] < 0) {
    return(NA)
  }
...
a.2.a.prime = function(x, theta) {
  ...
  sigma = theta[4]
  alpha(x, theta)^2 +
    ((lambda) / (sqrt(delta^2 + (x * sigma - mu)^2))) - 
    ((lambda * (x * sigma - mu)^2) / ((delta^2 + (x * sigma - mu)^2)^(3/2)))
}
...
A = function(x, theta) {
  ...
  (lambda / sigma) * sqrt(delta^2 + (x * sigma - mu)^2)
}
