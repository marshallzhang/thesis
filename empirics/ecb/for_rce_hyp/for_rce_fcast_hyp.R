if (Sys.getenv("SLURM_JOB_ID") != "") {
  ...
  for (i in 1:10) {
    obj = function(theta) {
#       source("/Users/marshall/Documents/senior/thesis/empirics/ecb/for_rce_hyp/to_source_parallel.R")
      source("to_source_parallel.R")
      q.beskos.unknown(theta, trans.hyp.fast, oldpar, nu, round(i^1.2)*100, steps, horizons/max(horizons), max(horizons), exact = F)
    }
    oldpar = DEoptim2(fn = obj,
                     ...
                     control = DEoptim.control(trace = TRUE, strategy = 6, parallelType = 1, limitCores= 8,
                                               ...
                                               parVar = list("q.beskos.unknown", "eta", "nu", "i", "A",
                                                             ...
                                                             "vec.seq", "a.2.a.prime", "forecast", "alpha", "hyp.drift")
                                                          ))[["optim"]]$bestmem
    print(oldpar)
  }
}