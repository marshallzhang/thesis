if (Sys.getenv("SLURM_JOB_ID") != "") {
  ...
  set.seed(3)
  for (i in 1:10) {
    oldpar = DEoptim(fn = function(theta) q.beskos.unknown(theta, trans.ou.fast, oldpar, nu, round(i^1.2)*100, steps, exact = F), 
                     lower = c(-1.5, 0, 0),
                     upper = c(1.5, 2.5, 2.5),
                     control = DEoptim.control(trace = TRUE, strategy = 6))[["optim"]]$bestmem
    print(oldpar)
  }
}